...
void PortInit(void)
{
    ...
    P1SKIP      |= 0x20;            // skip LED1 and LED2

    P2MDOUT     |= 0x02;            // RF ENABLE
	RFEN = EXTINGUISH;				// RF ENABLE    ILLUMINATE, EXTINGUISH
	P2MDOUT |= 0x08; 				// SDN P2.3 push-pull

    SFRPAGE     = CONFIG_PAGE;
    P0DRV       = 0x10;             // TX high current mode
    P1DRV       = 0x35;             // MOSI, SCK, NSS, LEDs high current mode
    P2DRV       = 0x08;             // SDN high current
    SFRPAGE     = LEGACY_PAGE;
