...
SEGMENT_VARIABLE(printCount, U16, APPLICATION_MSPACE);
...
void StateMachine(void)
{
    ...
    switch (DEMO_SR)
    {
        ...
        case DEMO_RX_NOSLEEP:
            if(fEZMacPRO_PacketReceived)
            {   /* Clear flag. */
                fEZMacPRO_PacketReceived = 0;
                /* Get packet then place the radio to sleep. */
		        vP2P_demo_GetPacketNoSleep();
	            LED1 = !LED1;
                // for (wWaitCnt = 0; wWaitCnt < 10; wWaitCnt++);
            }
	        break;
...
void vP2P_demo_GetPacketGoToSleep(void)
{
    ...
    EZMacPRO_Reg_Write(LFTMR0, 0x20);       // set the interval to 875ms
    EZMacPRO_Reg_Write(LFTMR1, 0x00);       // set the interval to 875ms
    EZMacPRO_Reg_Write(LFTMR2, 0xC4);       // enable the LFT, use the Internal Time Base
}

void vP2P_demo_GetPacketNoSleep(void)
{
	int i = 0;
	// EZMacProPayloadStruct * PayloadStruct = abRfPayload;
	EZMacProPayload_SensorNode * PayloadStruct = abRfPayload;
    /* Read out the payload. */
    EZMacPRO_RxBuf_Read(&bPacketLength, &abRfPayload[0]);
    /* Get the data. */
    //wPacketCounter = atoi(abRfPayload);
    // wPacketCounter = PayloadStruct->sensor_sequence;
  #ifdef TRACE_ENABLED
    // printf("[RX_NODE][DEMO_RX] Received packet[rssi:%03bu]. Content:%5u\n", rssiVal, printCount++);
    // printf("[RX_NODE]Received packet [ID:%02X] [rssi:%03bu] [Content:%u]\n", (U16)(PayloadStruct->sensor_ID[0]), rssiVal, (U8)(PayloadStruct->sensor_sequence));

	for(i = 0; i<bPacketLength ; i++)
    	printf("%02X ", (U16)(abRfPayload[i]));
    printf("\n\r");
